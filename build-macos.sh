...
cp disk_images/kafeinos.flp disk_images/kafeinos.dmg
echo "[okay] copied floppy image"

dd conv=notrunc if=source/bootload/bootload.bin of=disk_images/kafeinos.dmg || exit 1
...
dev=$(echo -n $(hdid -nobrowse -nomount disk_images/kafeinos.dmg))
...
mkisofs -quiet -V 'KAFEINOS' -input-charset iso8859-1 -o disk_images/kafeinos.iso -b kafeinos.dmg disk_images/ || exit 1
echo "[okay] converted floppy to ISO-8859-1 image"
echo "[done] build completed"
>>>>>>> 0bc5e7a (Critical update)
